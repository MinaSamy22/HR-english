                ...
                <div class=" mb-2 d-flex justify-content-between">
                    ...
                    {{-- <div class="">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">{{ __('Calender.home') }}</a>
                            </li>
                    ...
                    <div class="card-header">
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('performance-criteria.index') }}"
                                    class="btn btn-info btn-sm rounded-pill mr-2">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label
                                                    for="employee_name">{{ __('h_performance.search_by_employee') }}</label>
                                                <input type="text" name="employee_name" id="employee_name"
                                                    class="form-control"
                                                    placeholder="{{ __('h_performance.enter_employee_name') }}"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="d-flex">
                                                    ...
                                                    <a href="{{ route('performance.index') }}"
                                                        class="btn btn-secondary rounded-pill" style="margin-right: 10px;">
                        ...
                        @if(request()->anyFilled(['employee_name', 'month', 'year', 'status']))
                            <div class="alert alert-info">
                                ...
                                @if(request('status'))
                                        ...
                                        'h_performance.' . request('status')
                                    ) }}</strong>
                                @endif
                                ({{ $evaluations->total() }}
                                {{ $evaluations->total() != 1 ? __('h_performance.results') : __('h_performance.result') }})
                            </div>
                        @endif
                                    ...
                                    <tbody>
                                        @foreach($evaluations as $evaluation)
                                            <tr>
                                                ...
                                                <td>
                                                    <span
                                                        class="badge badge-{{ $evaluation->status == 'completed' ? 'success' : ($evaluation->status == 'reviewed' ? 'info' : 'warning') }}">
                                                ...
                                                <td>
                                                    <div class="btn-group" role="group">
                                                        <a href="{{ route('performance.show', $evaluation->id) }}"
                                                            ...
                                                            title="{{ __('h_performance.view') }}">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                        <a href="{{ route('performance.edit', $evaluation->id) }}"
                                                            class="btn btn-warning rounded-pill"
                                                            title="{{ __('h_performance.edit') }}">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                        <form method="POST"
                                                            action="{{ route('performance.destroy', $evaluation->id) }}"
                        ...
                        @else
                            <div class="text-center py-4">
                                @if(request()->anyFilled(['employee_name', 'month', 'year', 'status']))
                                    ...
                                    <p class="text-muted">{{ __('h_performance.adjust_search_criteria') }} <a
                                            href="{{ route('performance.index') }}">{{ __('h_performance.clear_filters') }}</a>.</p>
                                @else
                ...
                <div class="modal-footer">
                    <form id="deleteForm" method="POST">
                        ...
                        <button type="button" class="btn btn-secondary"
                            data-dismiss="modal">{{ __('h_performance.cancel') }}</button>
