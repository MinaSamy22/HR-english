        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="d-flex justify-content-between align-items-center flex-wrap">
                    <h1 class="m-0 mt-3 mb-3">{{ __('E_early.early_leave_requests') }}</h1>
                ...
                <div class="row">
                        ...
                        <div class="card card-primary">
                            ...
                            <form action="{{ route('employee.early.store') }}" method="POST" id="addForm">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label><i class="fas fa-calendar mr-1"></i>{{ __('E_early.date') }} <span
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label><i class="fas fa-clock mr-1"></i>{{ __('E_early.leave_time') }} <span
                                                        class="text-danger">*</span></label>
                                                <input type="time" name="requested_leave_time"
                                                    class="form-control @error('requested_leave_time') is-invalid @enderror"
                                    ...
                                    <!-- Reason -->
                                    <div class="form-group">
                                        <label><i class="fas fa-comment mr-1"></i>{{ __('E_early.reason_optional') }}<span
                                                class="text-danger">*</span></label>
                                ...
                                <div class="card-footer">
                                    <button class="btn btn-primary" id="submitBtn" type="submit"><i
                                            class="fas fa-paper-plane mr-1"></i>{{ __('E_early.submit_request') }}</button>
                                    <button type="reset" class="btn btn-secondary ml-2"><i
                                            class="fas fa-undo mr-1"></i>{{ __('E_early.reset_form') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="card card-info">
                            <div class="card-header">
                                <h3 class="card-title"><i class="fas fa-chart-line mr-2"></i>{{ __('E_early.summary') }}
                                </h3>
                            </div>
                            <div class="card-body">
                                ...
                                <ul class="list-group list-group-flush">
                                    <li class="list-group-item d-flex justify-content-between">
                                        <span><i
                                                class="fas fa-check mr-2 text-success"></i>{{ __('E_early.approved') }}</span>
                                        <span class="badge badge-success badge-pill">{{ $approved }}</span>
                                    </li>
                                    <li class="list-group-item d-flex justify-content-between">
                                        <span><i
                                                class="fas fa-hourglass-half mr-2 text-warning"></i>{{ __('E_early.pending') }}</span>
                                        <span class="badge badge-warning badge-pill">{{ $pending }}</span>
                                    </li>
                                    <li class="list-group-item d-flex justify-content-between">
                                        <span><i
                                                class="fas fa-times-circle mr-2 text-danger"></i>{{ __('E_early.rejected') }}</span>
                                                        ...
                                                        <td>
                                                            @if($request->status == 'pending')
                                                                <span class="badge badge-warning"><i
                                                                        class="fas fa-hourglass"></i>{{ __('E_early.pending') }}</span>
                                                            @elseif($request->status == 'approved')
                                                                <span class="badge badge-success"><i
                                                                        class="fas fa-check-circle"></i>{{ __('E_early.approved') }}</span>
                                                            @else
                                                                <span class="badge badge-danger"><i
                                                                        class="fas fa-times-circle"></i>{{ __('E_early.rejected') }}</span>
                                                            @endif
                                                        </td>
                                                        <td>{{ date('d M Y h:i A', strtotime($request->created_at)) }}</td>
                                                        <td>
                                                            @if($request->status == 'pending')
                                                                <form action="{{ route('employee.early.destroy', $request->id) }}"
                                                                    method="POST"
