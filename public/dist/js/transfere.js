        ...
        function updateSelectedCount() {
            ...
            if (count > 0) {
                ...
                transferBtn.textContent = count === 1 ?
                    ' Transfer Selected Employee' :
                    ` Transfer ${count} Selected Employees`;
            } else {
                transferBtn.disabled = true;
                transferBtn.innerHTML = '<i class="fas fa-random"></i> Transfer Selected Employees';
            }
        }
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            if (selectedEmployees.length === 0) {
                e.preventDefault();
                alert('Please select at least one employee to transfer.');
                return false;
            }

            if (!branchSelect.value) {
                e.preventDefault();
                alert('Please select a branch to transfer employees to.');
                return false;
            }
