...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!type) {
            errorMessage = "Please select a payroll type.";
            isValid = false;
        } else if (type === 'daily' && diffDays > 1) {
            errorMessage = "For daily payroll, start and end date must be the same.";
            isValid = false;
        } else if (type === 'weekly' && diffDays > 7) {
            errorMessage = "Weekly payroll cannot exceed 7 days.";
            isValid = false;
        } else if (type === 'monthly') {
    if (diffDays <= 7) {
        errorMessage = "Monthly payroll must exceed 7 days.";
        isValid = false;
    } else if (diffDays > 32) {
        errorMessage = "Monthly payroll cannot exceed 31 days.";
        isValid = false;
    }
}
